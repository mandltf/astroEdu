...
class _RasiListScreenState extends State<RasiListScreen> {
  ...
  Future<void> _initData() async {
    _items = AppConstants.rasiList.map((data) {
      return RasiModel(
        id: data['id'],
        name: data['name'],
        emoji: data['emoji'],
        color: data['color'],
        shortDesc: 'Rasi bintang ${data['name']}',
        description: 'Memuat dari Wikipedia...',
        bestTime: 'Belum diketahui',
        visibility: 'Belum diketahui',
        brightestStar: 'Belum diketahui',
        stars: 0,
        facts: [],
        season: 'Belum diketahui',
        wikiData: null,
      );
    }).toList();
  ...
  Future<void> _openItem(int index) async {
    ...
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => RasiDetailScreen(item: item, itemId: id, userId: _userId!),
      ),
    );
  ...
  Future<void> _openQuiz() async {
    ...
    if (!allRead) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Baca semua rasi bintang dulu!'), backgroundColor: AppTheme.marsRed),
      );
      return;
    }
    Navigator.push(
    ...
    return GestureDetector(
      onTap: () {
        if (unlocked) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('🔒 Selesaikan semua materi rasi bintang untuk membuka kuis!'),
              backgroundColor: AppTheme.marsRed,
            ),
          );
        }
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    unlocked
                        ? 'Uji pengetahuanmu tentang rasi bintang di langit malam'
                        : 'Selesaikan semua materi rasi bintang untuk membuka kuis',
                    style: TextStyle(
  ...
  Widget build(BuildContext context) {
    final filtered = _search.isEmpty
        ? _items
        : _items.where((r) => r.name.toLowerCase().contains(_search.toLowerCase())).toList();
