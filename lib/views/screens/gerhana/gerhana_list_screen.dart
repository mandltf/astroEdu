...
class _GerhanaListScreenState extends State<GerhanaListScreen> {
  ...
  Future<void> _initData() async {
    _items = AppConstants.gerhanaList.map((data) {
      return GerhanaModel(
        id: data['id'],
        name: data['name'],
        emoji: data['emoji'],
  ...
  Future<void> _openQuiz() async {
    final allRead = await DatabaseHelper.instance.allItemsRead(_userId!, 'gerhana', _items.length);
    if (!allRead) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Baca semua materi gerhana dulu!'), backgroundColor: AppTheme.marsRed),
      );
      return;
    }
    Navigator.push(
